...
//	isAccelerometerEnabled = YES;
	...
	hPadding = padding;
	lowerBound = [(CCMenuItem*)[_children objectAtIndex:0] contentSize].height / 2.0;
...
-(BOOL) ccTouchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	touchDown = true;
	moving = false;
	_selectedItem = [super itemForTouch:touch];
...
-(void) ccTouchEnded:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	if(!moving && _state == kCCMenuStateTrackingTouch)
		[super ccTouchEnded:touch withEvent:event];
	else if(_state == kCCMenuStateTrackingTouch)
		[self ccTouchCancelled:touch withEvent:event];
	moving = false;
	touchDown = false;
}
...
-(void) ccTouchMoved:(UITouch *)touch withEvent:(UIEvent *)event
{
	...
	else if(!moving && _state == kCCMenuStateTrackingTouch)
	{
...
-(void) moveItemsLeftBy:(float) offset
{
	...
	for(CCMenuItem<CCRGBAProtocol>* item in _children)
	{
		[item setPosition:ccpAdd([item position], ccp(offset, 0))];
	}
	
	CCMenuItem* leftItem = [_children objectAtIndex:0];
	if([leftItem position].x + [self position].x + [leftItem contentSize].width / 2.0  < 0)
	{
		...
		CCMenuItem* lastItem = [_children objectAtIndex:[_children count] - 1];
...
-(void) moveItemsRightBy:(float) offset
{
	...
	if([lastItem position].x + [self position].x - [lastItem contentSize].width / 2.0 > 480)
	{
		...
		CCMenuItem* firstItem = [_children objectAtIndex:0];
		...
		[_children insertObject:lastItem atIndex:0];
...
-(void) updateAnimation
{
	...
	const float winhalfWidth = (float)winSize.width/2;
    
	for(CCMenuItem<CCRGBAProtocol>* item in _children)
	{
		float distance = fabsf([item position].x - winhalfWidth + [self position].x);
		
		if(distance > winhalfWidth)
			distance = winhalfWidth;
